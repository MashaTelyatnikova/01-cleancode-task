...
namespace CleanCode
{
	public static class RefactorMethod
	{
		private static void SaveData(string file, byte[] data)
		{
		    ...
		    var fileStream = GetFileStream(file, Path.GetExtension(file));
		    var backupFileStream = GetFileStream(file, "bkp");
			
            WriteDataToFileStream(data, fileStream);
            WriteDataToFileStream(data, backupFileStream);

			var fileTime = file + ".time";
		    var timeFileStream = GetFileStream(fileTime, Path.GetExtension(fileTime));
			var bytesCurrentTime = BitConverter.GetBytes(DateTime.Now.Ticks);
			
            WriteDataToFileStream(bytesCurrentTime, timeFileStream);
		}

	    public static FileStream GetFileStream(string path, string extension)
	    {
	        return new FileStream(Path.ChangeExtension(path, extension), FileMode.OpenOrCreate );
	    }
